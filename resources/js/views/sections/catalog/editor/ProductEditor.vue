<template>
  <v-container fluid class="fill-height bg-primary-lighten-3 d-flex justify-center overflow-auto">
    ...
    <v-container v-if="editingProduct.variations" class="border rounded-lg mt-5">
      <v-container class="py-8">
        <span>Variantes</span>
        <v-container v-for="variation in editingProduct.variations" :key="variation.id"
          class="d-flex align-top flex-wrap">
          <VariationEditor
          :variation="variation"
    ...
    <v-container class="d-flex justify-end">
      ...
      <v-btn variant="outlined" class="mx-2" @click="$emit(
        'save',
        { product: editingProduct, modifiedVariations: [...modifiedVariations] }
...
export default {
  ...
  methods: {
    ...
    updateVariation(updatedVariation) {
      ...
      const variationIndex = this.product.variations
        .findIndex((variation) => variation.id === updatedVariation.id);
